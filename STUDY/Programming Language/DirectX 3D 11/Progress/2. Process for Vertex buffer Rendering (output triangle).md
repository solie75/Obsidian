...
4. 정점 버퍼 그리기 테스트(이하 Test)의 초기화
	...
	3. TestInit( )
		...
		6. shader file 생성 및 경로 가져오기
			...
			4. 
			5. 
			6. (float3형 위치, float4형 색 정보를 가진)입력 구조체(VS_IN), (float4형 위치 정보를 가진)출력 구조체(VS_OUT)를 생성한다.
			7. vertex shader (VS_Test) 와 pixel shader (PS_Test)을 생성한다 . 이 때 pixel shader 는  vertex shader 의  출력 값을 입력값으로 받는다.
			8. 
