...
4. 코너에서 머리가 회전하는 모션추가
5. 플레이어의 칼에 충돌체 추가.
6. 몸체의 충돌체 를 몸체의 형태와 맞게 할 수 있는지에 대해 생각할 것
7. 따로 Desert Boss stage 만들고 그 스테이지에 현재의 test stage 내용을 옮길것
8. 파티클 추가
9.  hit 시에 몸체가 흰색과 빨간색으로 변하는것 만들기 
10. 각 몸체의 hp 와 화면상 보일 총합 hp 에 대해 구상하기
11. 몸체가 벽 또는 서로에 대해 부딫혀 튕기는 공격패턴 추가
12. 몸체가 맞을 때 살짝 움찍하는 로직 구현
13. 플레이어의 hp 구현
14. LavaSplash 와 충돌 할때 플레이어의 발에서만 충돌 되어야 한다.(현재 머리로도 충돌됨)
15. circle 몸체에서 전방위 50 개 fireball 발사시에 프레임 드랍 현상 해결하기
16. debug 들이 모두 같은 z 축을 가지고 출력되어  상대적으로 먼 z 값영역이 잘리는 현상이 없게 한다.
17. 카메라 플레이어에 고정시키기
18. 카메라가 배경 화면 밖으로 안나가게 하기
19. 충돌체에 offset 적용시키기 => 완료 (08-20-18-04)
